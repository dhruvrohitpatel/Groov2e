// Tweaks panel — bottom-right floating. Controls theme, density, wave style, mixer, phone preview.

function TweaksPanel({ tweaks, setTweak, open, onClose, theme }) {
  if (!open) return null;
  return (
    <div style={{
      position:'fixed', bottom: 24, right: 24, zIndex: 50,
      width: 280,
      background: theme.tweakBg,
      ...
      <div style={{padding:'12px 14px', display:'flex', flexDirection:'column', gap: 14}}>
        ...
        <TweakRow label="waveform">
          <Seg options={[
            {v:'bars', l:'bars'},
            {v:'filled', l:'filled'},
            {v:'line', l:'line'},
          ]} value={tweaks.waveStyle} onChange={v => setTweak('waveStyle', v)}/>
        </TweakRow>
...
function TweakRow({ label, children }) {
  return (
    <div>
      <div style={{fontFamily:'var(--mono)', fontSize:9.5, opacity:0.55, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom: 5}}>{label}</div>
      {children}
    </div>
  );
}
...
function Seg({ options, value, onChange }) {
  return (
    <div style={{display:'flex', border:'1px solid rgba(20,18,16,0.15)', borderRadius:6, padding:2, background:'rgba(20,18,16,0.03)'}}>
      {options.map(o => (
